...
% We start by defining the problem. This means specifying the time horizon, the constraint set, or "safe set", and the target set.
N = 5;
...
surf(ax1, s, s, reshape(result.prob(:, 1), 100, 100).', 'EdgeColor', 'none');
caxis([0 1]);
view([0 90]);
...
surf(ax2, s, s, reshape(results.Pr(1, :), 100, 100), 'EdgeColor', 'none');
caxis([0 1]);
view([0 90]);
...
surf(ax3, s, s, abs(reshape(results.Pr(1, :), 100, 100) - reshape(result.prob(:, 1), 100, 100).'), 'EdgeColor', 'none');
caxis([0 1]);
view([0 90]);
...
surf(ax4, s, s, reshape(results_rff.Pr(1, :), 100, 100), 'EdgeColor', 'none');
caxis([0 1]);
view([0 90]);
...
surf(ax5, s, s, abs(reshape(results_rff.Pr(1, :), 100, 100) - reshape(result.prob(:, 1), 100, 100).'), 'EdgeColor', 'none');
caxis([0 1]);
view([0 90]);
